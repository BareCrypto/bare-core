...
VERSION=
commit=false
url=https://github.com/crypto-node
proc=2
mem=2000
lxc=true
...
-u|--url	Specify the URL of the repository. Default is https://github.com/crypto-node
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit bare=${COMMIT} --url bare=${url} ../bare/contrib/gitian-descriptors/gitian-linux.yml
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit bare=${COMMIT} --url bare=${url} ../bare/contrib/gitian-descriptors/gitian-win.yml
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit bare=${COMMIT} --url bare=${url} ../bare/contrib/gitian-descriptors/gitian-osx.yml
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit bare=${COMMIT} --url bare=${url} ../bare/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../bare/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/bare-*.tar.gz build/out/src/bare-*.tar.gz ../bare-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../bare/contrib/gitian-descriptors/gitian-win-signer.yml
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../bare/contrib/gitian-descriptors/gitian-osx-signer.yml

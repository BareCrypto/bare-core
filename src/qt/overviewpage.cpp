...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BARE)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, QString& sBAREPercentage)
{
    int nPrecision = 2;
    ...
    sBAREPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    bool showBAREAvailable = settingShowAllBalances || phrAvailableBalance != nTotalBalance;
    bool showWatchOnlyBAREAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showBAREPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyBAREPending = watchUnconfBalance != 0;
    bool showBARELocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyBARELocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showBAREAvailable || showWatchOnlyBAREAvailable);
    ui->labelBalanceText->setVisible(showBAREAvailable || showWatchOnlyBAREAvailable);
    ui->labelWatchAvailable->setVisible(showBAREAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showBAREPending || showWatchOnlyBAREPending);
    ui->labelPendingText->setVisible(showBAREPending || showWatchOnlyBAREPending);
    ui->labelWatchPending->setVisible(showBAREPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showBARELocked || showWatchOnlyBARELocked);
    ui->labelLockedBalanceText->setVisible(showBARELocked || showWatchOnlyBARELocked);
    ui->labelWatchLocked->setVisible(showBARELocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BARE")
    updateDisplayUnit();
}
